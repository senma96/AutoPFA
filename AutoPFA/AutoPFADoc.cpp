...
// #include "DlgDataFormatDll.h"  // 已�?????�?缺失�????DLL
// #include "StartCADObj.h"       // 已�?????�?缺失AutoCAD??��??
...
BEGIN_MESSAGE_MAP(CAutoPFADoc, CDocument)
	...
	//}}AFX_MSG_MAP
	ON_COMMAND(ID_OUTPUTCONTROL, OnOutPutControl)   // �?�?�?VC6.0??��?��??�?
	ON_COMMAND(ID_OUTPUTRESULT, OnOutPutResult)     // �?�?�?VC6.0??��?��??�?
END_MESSAGE_MAP()
...
{
	// TODO: Add your command handler code here
	m_scenarioManager.GetCurScenario()->GetCalcManager()->AnalysisType(0);//��õ�ǰ�����ļ�������������ü�������Ϊ0��������˲̬
	m_workSpaceRef.Trans(TRUE);
...
void CAutoPFADoc::OnExportCalcFile() 
{
	...
	if(bRet)
	{
		...
		if(index != -1)
		{
			...
			static TCHAR BASED_CODE szFilter[] = _T("PFA�����ļ�(*.calc)|*.calc|All Files (*.*)|*.*||");
...
void CAutoPFADoc::OnRunModel() 
{
	...
	if(GetModel(model,pScenario))
	{
		...
		if(index == -1)
		{
			AfxMessageBox(_T("���ȱ����ĵ�"));
			return;
		}
		strDocName = strDocName.Left(index);
		strPath = strDocName;
		strDocName = strDocName +_T(".imp");
//		OnSaveDocument(strDocName);//������и�ʽ��һ��������֣�����������
...
/************************************************************************
�汾: 8.0  ע��ʱ��: [2009/4/29]  ����: [Zhufy] ������: 
����:     ��ȡTableFormat.mdb���ݿ⣬���ݱ������������õ���ʾ���ֶ�ˢ�½���                                                         
����:                                                               
���?:                                                              
< �޸��� >    < ʱ�� >     < �汾 >        < ���� >                                                                               
************************************************************************/
void CAutoPFADoc::OnOutPutControl()
{
    ...
    if( dlg.DoModal( str.GetBuffer( str.GetLength() ) ) == IDOK )
	{
		...
		CString strDBName[4] = { _T("tpfaoutPipe") ,      //�ܵ���
			                     _T("tpfaoutJun") ,       //�ڵ��?
								 _T("tpfaoutTransient") , //˲���?
								 _T("tpfaoutMaxMinTran")  //�����С˲���
		                       };
			...
			for ( i = 0 ; i < 4 ; i++ )
			{
				...
				while( !pRs[i]->IsEmpty() && !pRs[i]->IsEOF() )
				{
					...
					while( !pRsTemp->IsEmpty() && !pRsTemp->IsEOF() )
					{
						...
						{
							strUnitTemp = pRsTemp->GetString( _T("UnitSymbol") );//��ǰ��λ
							break;
						}
						pRsTemp->MoveNext();
					}
